...
|---|---|---|---|
| v2.8.8 | 2026-09-01 | **Item D bills the calendar month, not the contracted-units cycle.** Monthly Pricing and Flat Rate cover the 1st through the last day of the month (Confluence *Automated Representation Billing Processes by Agreement Price Type*), but every Item D check measured against the `pStart`/`pEnd` cycle, which opens in the PRIOR month (Aug 2026 = 07-27 → 08-30). A shutdown running Mon 07-27 – Sun 08-02 — July time July's invoice already covered — was therefore counted against August's flat rate, and the audit demanded a credit that did not belong there. New `flatRateBillingMonth()` derives the calendar month containing the period end; shutdown credits, the 100%-full-month rule and MP/FR pro-ration all read it (`FR_MONTH` in `_doAnalyze`, `frMonth` in `buildExpectedSnapshot`). CU keeps the cycle. Second cause, same symptom: dates parsed as UTC midnight were read back with LOCAL day-of-week, shifting every week boundary a day west of Greenwich — in Eastern time that also dropped the same credit from July, so the fix had to cover both. New `cd`/`cdDay`/`cdAdd`/`cdISO` primitives; `countFullShutdownWeeks`, `countWeekdays`, `forEachMonFriWeek`, `getApplicableShutdowns` converted. Also: MP/FR pro-ration was dividing by the `Total Weekdays in Period` field (25 for the Aug cycle) instead of the month's 21, understating mid-month amounts ~16%; and a verified credit row rendered the literal `${fullWeeks} wks × 20%` (single quotes nested in a template literal). Adds `npm test` — `scripts/test-flat-rate-period.mjs` runs the functions extracted from `index.html` (22 assertions) and is green under 5 timezones; also driven in headless Chromium at `America/New_York` against the real Hall 50 rows. | `442806e` |
